...
type RestOf<T extends ITrip | NewTrip> = Omit<T, 'startDate' | 'endDate'> & {
...
export class TripService {
  ...
  protected convertDateFromClient<T extends ITrip | NewTrip | PartialUpdateTrip>(trip: T): RestOf<T> {
    return {
      ...trip,
      startDate: trip.startDate?.format(DATE_FORMAT) ?? null,
      endDate: trip.endDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restTrip: RestTrip): ITrip {
    return {
      ...restTrip,
      startDate: restTrip.startDate ? dayjs(restTrip.startDate) : undefined,
      endDate: restTrip.endDate ? dayjs(restTrip.endDate) : undefined,
    };
  }
